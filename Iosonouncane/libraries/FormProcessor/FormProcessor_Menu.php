...
class FormProcessor_Menu
{
    public function processor($post){
        $errors = array();
        
        $menu_id = ($post['menu_id'] != 0) ? (int)$post['menu_id'] : '';
        if($post['menu_id'] == 0) $errors['menu_id'] = 'Errore "menu_id" è vuoto! Selezionare un id valido';
        ...
        if(empty($post['menu_position'])) $errors['menu_position']= 'Attenzione! La posizione del punto menu non è stata impostata';
...
/* JAVASCRIPT */    exit('<script type="text/javascript">
            ...
            $userInputs = array('menu_id' => $menu_id , 'menu_name' => $menu_name, 'menu_title' => $menu_title, 'menu_visible' => $menu_visible, 
